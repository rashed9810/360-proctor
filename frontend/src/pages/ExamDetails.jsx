  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      setExam({
        ...
        description: 'Comprehensive exam covering all topics from the semester including algebra, calculus, and statistics.',
        date: '2023-06-15',
        time: '10:00 AM',
        duration: 120,
        status: 'published',
        ...
        instructions: 'Please ensure you are in a quiet environment. No books or notes are allowed. You must keep your camera and microphone on throughout the exam.'
      });
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-gray-900">{exam.duration} {t('minutes')}</p>
                </div>
          ...
          <div className="card">
            ...
            <div className="space-y-3">
              <div className="flex items-center">
                <div className={`flex-shrink-0 h-5 w-5 rounded-full ${exam.proctoring.faceDetection ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                <p className="ml-3 text-sm text-gray-700">{t('faceDetection')}</p>
              </div>
              
              <div className="flex items-center">
                <div className={`flex-shrink-0 h-5 w-5 rounded-full ${exam.proctoring.multipleFaceDetection ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                <p className="ml-3 text-sm text-gray-700">{t('multipleFaceDetection')}</p>
              </div>
              
              <div className="flex items-center">
                <div className={`flex-shrink-0 h-5 w-5 rounded-full ${exam.proctoring.eyeTracking ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                <p className="ml-3 text-sm text-gray-700">{t('eyeTracking')}</p>
              </div>
              
              <div className="flex items-center">
                <div className={`flex-shrink-0 h-5 w-5 rounded-full ${exam.proctoring.audioDetection ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                <p className="ml-3 text-sm text-gray-700">{t('audioDetection')}</p>
              </div>
              
              <div className="flex items-center">
                <div className={`flex-shrink-0 h-5 w-5 rounded-full ${exam.proctoring.tabSwitchDetection ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                <p className="ml-3 text-sm text-gray-700">{t('tabSwitchDetection')}</p>
              </div>
              
              <div className="flex items-center">
                <div className={`flex-shrink-0 h-5 w-5 rounded-full ${exam.proctoring.phoneDetection ? 'bg-green-500' : 'bg-gray-300'}`}></div>
