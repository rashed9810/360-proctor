 ...
 */
const PageHeader = ({ title, showBackButton = false, backTo, backLabel, actions, children }) => {
  const { t } = useTranslation();
  const location = useLocation();

  // Only show back button if explicitly requested or if we're not on the main dashboard
  const shouldShowBackButton =
    showBackButton || (location.pathname !== '/' && location.pathname !== '/dashboard');

  return (
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
      <div className="flex items-center gap-3">
        {shouldShowBackButton && <BackButton to={backTo} label={backLabel} />}
