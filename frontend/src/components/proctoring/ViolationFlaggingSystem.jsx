...
const ViolationFlaggingSystem = ({
   ...
   */
  const processViolation = useCallback((violationData) => {
   ...
   * Check if violation should be auto-flagged
   */
  const checkAutoFlag = useCallback((violation, violationType) => {
    const recentSameTypeViolations = violations.filter(v => 
      v.type === violation.type && 
      Date.now() - new Date(v.timestamp).getTime() < 300000 // Last 5 minutes
    );
   ...
   * Flag a violation
   */
  const flagViolation = useCallback((violation, reason) => {
   ...
   */
  const updateTrustScore = useCallback((violation) => {
   ...
   */
  const updateRiskLevel = useCallback((score) => {
   ...
   */
  const analyzeViolationPatterns = useCallback(async (currentViolations) => {
          ...
          <CardContent className="p-4 text-center">
            <div className="flex items-center justify-center mb-2">
              <div className={`w-16 h-16 rounded-full flex items-center justify-center ${
                trustScore > 80 ? 'bg-green-100 text-green-600' :
                trustScore > 60 ? 'bg-yellow-100 text-yellow-600' :
                trustScore > 40 ? 'bg-orange-100 text-orange-600' :
